...
export default function SeriesIndex({ series, filters = {}, statusOptions = [] }) {
    ...
    const applyFilters = (event) => {
        ...
        router.get(route('admin.series.index'), localFilters, {
            preserveScroll: true,
            preserveState: true,
            replace: true,
        });
    ...
    const resetFilters = () => {
        ...
        router.get(route('admin.series.index'), reset, {
            preserveScroll: true,
            replace: true,
        });
    };
        ...
        <DashboardLayout>
            ...
            <div className="mx-auto max-w-full space-y-8">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <p className="text-sm font-semibold uppercase tracking-wide text-brand-600">Series Management</p>
                        <h1 className="text-3xl font-bold text-slate-900">Manage Series</h1>
                        <p className="mt-1 text-sm text-slate-500">Create and manage article series to organize related content.</p>
                    </div>
                    <Link
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-100 text-sm">
                                {seriesRows.map((seriesItem) => (
                                    <tr key={seriesItem.id} className="transition hover:bg-slate-50/60">
                                        <td className="px-6 py-4">
                                            <div className="font-semibold text-slate-900">{seriesItem.name}</div>
                                            <div className="text-xs uppercase text-slate-400">/{seriesItem.slug}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <span className="inline-flex h-2 w-2 rounded-full bg-blue-500" />
                                                {seriesItem.articles_count} total
                                                <span className="text-slate-400">|</span>
                                                {seriesItem.published_articles_count} published
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <StatusBadge status={seriesItem.published_articles_count > 0 ? 'active' : 'inactive'} />
                                        </td>
                                        <td className="px-6 py-4 text-slate-600">
                                            <div className="flex items-center gap-2">
                                                <span className={`inline-flex h-2 w-2 rounded-full ${seriesItem.has_featured_image ? 'bg-green-500' : 'bg-slate-300'}`} />
                                                {seriesItem.has_featured_image ? 'Featured' : 'No featured'}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-slate-600">
                                            {seriesItem.created_at ? new Date(seriesItem.created_at).toLocaleDateString() : '-'}
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-end gap-2">
                                                <Link
                                                    href={route('admin.series.show', seriesItem.id)}
                                                ...
                                                <Link
                                                    href={route('admin.series.edit', seriesItem.id)}
                    ...
                    {seriesRows.length === 0 && (
                        <div className="px-6 py-12 text-center">
                            ...
                            <h3 className="mt-4 text-base font-semibold text-slate-900">No series found</h3>
                            <p className="mt-2 text-sm text-slate-500">Try adjusting your filters or create a new series.</p>
                        </div>
                    )}
